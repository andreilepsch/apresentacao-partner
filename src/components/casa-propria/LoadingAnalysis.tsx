...
import { Home, Calculator, TrendingUp, Target, CheckCircle } from "lucide-react";
...
const LoadingAnalysis = ({ durationMs = 7000, onComplete }: LoadingAnalysisProps) => {
  ...
  const steps = [
    { icon: Calculator, text: "Analisando sua situação financeira", color: "#193D32" },
    { icon: Home, text: "Calculando opções", color: "#B78D4A" },
    { icon: TrendingUp, text: "Projetando cenários de compra", color: "#355F4D" },
    { icon: Target, text: "Montando seu planejamento", color: "#D4B570" }
  ];
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  ...
                  {isCompleted ? (
                    ...
                    <StepIcon 
                      className={`w-5 h-5 ${
                        isCurrent ? 'text-[#193D32]' : 'text-gray-400'
                      }`} 
                    />
                  )}
                </div>
