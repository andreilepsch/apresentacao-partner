...
const AposentadoriaStep14 = () => {
  ...
  const handleFinalizarJornada = async () => {
    ...
    try {
      ...
      const savedFormData = JSON.parse(localStorage.getItem('aposentadoriaFormData') || '{}');
      
      const formData = {
        monthlyInvestment: savedFormData.monthlyInvestment || 'N/A',
        targetRetirement: savedFormData.targetRetirement || 'N/A',
        currentAge: savedFormData.currentAge || 'N/A',
        retirementAge: savedFormData.retirementAge || 'N/A'
      };
      ...
      const currentAgeValue = parseInt(savedFormData.currentAge || '0');
      const retirementAgeValue = parseInt(savedFormData.retirementAge || '0');
      
      // Usar os dados dos ciclos apresentados na UI (getRetirementCycleData)
      const cycles = getRetirementCycleData(monthlyInvestmentValue, currentAgeValue, retirementAgeValue);
      const yearsToRetirement = retirementAgeValue - currentAgeValue;
      const totalInvested = monthlyInvestmentValue * 12 * yearsToRetirement;
      
      // Usar o último ciclo para calcular os resultados finais
      const finalCycle = cycles[cycles.length - 1];
      const finalPatrimonyValue = parseFloat(finalCycle?.patrimony?.replace(/[R$\s.]/g, '').replace(',', '.') || '0');
      const finalIncomeValue = parseFloat(finalCycle?.income?.replace(/[R$\s.]/g, '').replace(',', '.') || '0');
      
      const results = {
        finalProjection: finalCycle?.patrimony || formatCurrency(0),
        monthlyIncome: finalCycle?.income || formatCurrency(0),
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#193D32] to-[#2A5A47] font-manrope">
      <div className="container mx-auto px-4 py-16 max-w-[1140px]">
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-between items-center max-w-4xl mx-auto">
          <Button
            variant="outline"
            onClick={() => navigateWithPreview("/aposentadoria/step13")}
...
export default AposentadoriaStep14;